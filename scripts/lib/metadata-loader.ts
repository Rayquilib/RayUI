...
export class MetadataLoader {
  ...
  async loadMetadata(): Promise<Map<string, BlockMetadata>> {
    try {
      ...
      const rayuiMetadata = metadataModule.rayuiMetadata;

      if (!Array.isArray(rayuiMetadata)) {
        throw new Error(
          `Expected rayuiMetadata to be an array in ${this.config.metadataFile
          }, got ${typeof rayuiMetadata}`
        );
      }

      console.log(`Loaded ${rayuiMetadata.length} metadata entries`);

      const metadataMap = new Map<string, BlockMetadata>();
      const errors: string[] = [];

      for (const [index, item] of rayuiMetadata.entries()) {
      ...
      if (error instanceof Error) {
        if (error.message.includes("ENOENT")) {
          throw new Error(
            ...
            `Please ensure the file exists and contains exported rayuiMetadata array.`
          );
