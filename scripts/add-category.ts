...
`;
  ...
  // Add to preRayUICategoriesMetadata array
  const metadataMatch = categoriesContent.match(/(const preRayUICategoriesMetadata: Omit<RayUICategoryMetadata, "count">\[\] = \[)([\s\S]*?)(\];)/);
  if (metadataMatch) {
